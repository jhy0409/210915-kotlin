...
class MainActivity : AppCompatActivity() {
    ...
    class HeaderViewHodler(view: View): RecyclerView.ViewHolder(view) {
        val headerView=view.itemHeaderView
    }
    class DataViewHodler(view: View): RecyclerView.ViewHolder(view) {
    ...
    inner class MyAdapter(val list: MutableList<ItemVO>): RecyclerView.Adapter<RecyclerView.ViewHolder>() {
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
            if(viewType==ItemVO.TYPE_HEADER){
                ...
                return HeaderViewHodler(inflater.inflate(R.layout.item_header, parent, false))
            } else {
                val inflater=LayoutInflater.from(parent.context)
                return DataViewHodler(inflater.inflate(R.layout.item_main, parent, false))
            }
        }
        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            ...
            if(itemVO.type===ItemVO.TYPE_HEADER) {
                val viewHolder = holder as HeaderViewHodler
                val headerItem=itemVO as HeaderItem
                viewHolder.headerView.setText(headerItem.date)
            } else {
                val viewHolder = holder as DataViewHodler
...
abstract class ItemVO {
    ...
    companion object {
        val TYPE_HEADER=0 // date
        val TYPE_DATA=1 // todo
    }
}
